...
$group = $item.name
    ...
    select @{n='Group';e={$group}}, @{n='Description';e={$_.description}}, @{n='Info';e={$_.Info}} | Export-CSV -append $members –NoTypeInformation
}
...
(gc $members) | % {$_ -replace '-----', ""} | ? {$_.trim() -ne "" } | out-file -append $membersof -Fo -En ascii

Import-Csv $membersof | select * -ExcludeProperty Description,Info | Export-CSV -append $membersof1 –NoTypeInformation
